...
def play_game(agent1, agent2, seed, time1=0.2, time2=0.2, max_plies=500):
    ...
    while not state.isGameOver and plies < max_plies:
        if state.currentPlayer == 0:
            agent, t = agent1, time1
        else:
            agent, t = agent2, time2
        move = choose_move(state, agent, rng, t)
...
def battle(agent1, agent2, games=20, seed=42, time1=0.2, time2=0.2):
    result = BattleResult(games=games)
    total_plies = 0

    for i in range(games):
        if i % 2 == 0:
            first_agent, first_time = agent1, time1
            second_agent, second_time = agent2, time2
        else:
            first_agent, first_time = agent2, time2
            second_agent, second_time = agent1, time1
...
def main():
    ...
    parser.add_argument("--time", type=float, default=0.2, help="seconds per AI move (used as default for --time1/--time2)")
    parser.add_argument("--time1", type=float, default=None, help="seconds per move for agent1 (overrides --time)")
    parser.add_argument("--time2", type=float, default=None, help="seconds per move for agent2 (overrides --time)")
    ...
    started = time.time()
    result = battle(args.agent1, args.agent2, args.games, args.seed, time1, time2)
    elapsed = time.time() - started

    print(f"{args.agent1}(t={time1}s) vs {args.agent2}(t={time2}s)")
